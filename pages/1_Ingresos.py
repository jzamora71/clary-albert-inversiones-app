...
# without wiping any data already saved.
init_session_state()
db.init_db()
...
st.title("📥 Ingresos")
st.caption(f"{COMPANY_NAME} - Registro de ingresos (guardado permanentemente)")
...
if st.button("Agregar ingreso"):
    if ingreso_concepto.strip() and ingreso_monto > 0:
        db.add_ingreso(ingreso_concepto.strip(), ingreso_fecha.isoformat(), float(ingreso_monto))
        st.success("Ingreso agregado y guardado.")
    else:
...
if ingresos:
    ingresos_df = pd.DataFrame(ingresos)[["concepto", "fecha", "monto"]]
    ingresos_df.columns = ["Concepto", "Fecha", "Monto"]
